...
#         act.load_state_dict(torch.load("/home/zhaowenbo/Documents/SpaceRobot/SpaceRobotEnv/RL_algorithms/Torch/on-policy/onpolicy/scripts/results_server/SpaceRobotEnv/SpaceRobotDualArmWithRot/mappo/check/run12/models/actor_agent"+str(i)+".pt"))
#         actors.append(act)
...
def main(args):
    ...
    with torch.no_grad():
        ...
        for eval_step in range(all_args.episode_length):
            ...
            obs, eval_rewards, done, infos = env.step(np.stack(action).squeeze())
            eval_episode_rewards.append(eval_rewards)
        #     image = env.env.render("rgb_array")
        #     frames.append(image)
        #     # print(frames)
